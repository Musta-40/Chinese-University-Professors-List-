...
Enhanced version for Chinese university websites with Groq AI fallback.
"""
...
class SmartFacultyProfileScraper:
    ...
    def extract_email(self, soup) -> Optional[str]:
        ...
        # First, try the specific pattern mentioned: ">E-mail: ...@ecust.edu.cn"
        page_html = str(soup)
    ...
    def extract_name(self, soup) -> str:
        """Extract faculty name from page - prioritize title tag for Chinese sites"""
        # 1) Priority: <title> tag (as mentioned by user)
        ...
        # 2) Look for name in specific patterns
        name_patterns = [
            r'姓名[：:]\s*([^\s,，]+)',
            r'教师姓名[：:]\s*([^\s,，]+)',
            r'Name[：:]\s*([^\s,，]+)',
    ...
    def extract_research_interests_traditional(self, soup) -> str:
        """Traditional rule-based extraction of research interests - optimized for Chinese"""
        # Remove script/style
...
def main():
    parser = argparse.ArgumentParser(
        description='Smart Faculty Profile Scraper with AI Support (Optimized for Chinese Universities)'
    )
