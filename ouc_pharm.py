...
class AIExtractor:
    ...
    def extract_research_from_content(self, page_content: str, faculty_name: str = "") -> str:
        ...
        prompt = f"""
        Analyze this faculty webpage and extract or infer their research interests.
        ...
        Instructions:
        1. FIRST, look for sections labeled: 研究方向, 研究领域, 研究兴趣, 主要研究领域, Research Area, Research Direction
        2. If found, extract that content directly (but stop at 个人简介, 教育背景, 工作经历, etc.)
        3. If NOT found, try to INFER research interests from:
           - Publication titles (论文, Publications)
           - Project titles (if any)
           - Research descriptions in the text
        4. Focus on main research themes, methodologies, and areas of study
        5. Do NOT include: biographical info (个人简介), education (教育背景), work experience, awards
...
class SmartFacultyProfileScraper:
    ...
    DEPARTMENT_EMAIL_PATTERNS = [
        ...
        # Common department abbreviations (case insensitive)
    ...
    def __init__(self, args):
        ...
        # Not tracking duplicate emails anymore
    ...
    def extract_from_ouc_format(self, soup) -> Tuple[Optional[str], Optional[str], Optional[str]]:
        """Extract from Ocean University of China specific format"""
        name = None
        email = None
        research = None
        ...
        if title:
            ...
            if title_text and len(title_text) <= 10 and not any(sep in title_text for sep in ['-', '|', '/']):
                # Check if it looks like a Chinese name
                if re.match(r'^[\u4e00-\u9fa5]{2,4}$', title_text):
                    name = title_text
                    logger.debug(f"Extracted name from title: {name}")
        
        # Extract from META description (both email and research)
        ...
        if meta_desc:
            ...
            # Extract email - look for 电子邮箱：xxx@ouc.edu.cn
            email_patterns = [
                r'电子邮箱[：:]\s*([\w\.-]+@ouc\.edu\.cn)',
                r'邮箱[：:]\s*([\w\.-]+@ouc\.edu\.cn)',
                r'Email[：:]\s*([\w\.-]+@ouc\.edu\.cn)',
                r'E-mail[：:]\s*([\w\.-]+@ouc\.edu\.cn)',
                r'([\w\.-]+@ouc\.edu\.cn)'
            ]
    ...
    def extract_name(self, soup) -> str:
        ...
        name, _, _ = self.extract_from_ouc_format(soup)
        if name:
            return name
        ...
        # Look for name patterns in page text
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                if name and 1 < len(name) <= 10:
                    logger.debug(f"Extracted name from pattern: {name}")
                    return name
    ...
    def extract_email(self, soup) -> Optional[str]:
        ...
        # Try OUC format first
        _, email, _ = self.extract_from_ouc_format(soup)
        if email:
            return email
        ...
        page_text = soup.get_text(" ")
        # Find all emails
        all_emails = re.findall(r'[\w\.-]+@[\w\.-]+\.edu\.cn', page_text, re.IGNORECASE)
        ...
        logger.debug("Could not find personal email (only department emails found)")
        return None

    def extract_research_interests_comprehensive(self, soup) -> str:
        """Comprehensive research extraction trying all methods"""
        
        # Method 1: Try OUC format (META description)
        _, _, research = self.extract_from_ouc_format(soup)
        if research and len(research) > 20:
            logger.debug(f"Using research from OUC format: {len(research)} chars")
        ...
        page_html = str(soup)
        html_patterns = [
            (r'研究方向[：:]\s*(.+?)(?:个人简介|教育背景|$)', re.IGNORECASE | re.DOTALL),
            (r'研究领域[：:]\s*(.+?)(?:个人简介|教育背景|$)', re.IGNORECASE | re.DOTALL),
            (r'主要研究[：:]\s*(.+?)(?:个人简介|教育背景|$)', re.IGNORECASE | re.DOTALL),
        ]
        ...
        logger.debug("Could not extract research interests")
        return ""
    ...
    def scrape_profile(self, url: str) -> Optional[Dict[str, str]]:
        ...
        for attempt in range(self.args.retries + 1):
            try:
                ...
                logger.info(f"Extracted - Name: {name}, Email: {email or 'Not found'}")

                # Try traditional extraction first
                logger.info(f"Attempting traditional extraction for research interests")
                research_interests = self.extract_research_interests_comprehensive(soup)
                
                # Log what we found
                if research_interests:
                    logger.info(f"Traditional extraction successful: {len(research_interests)} chars")
                else:
                    logger.info(f"Traditional extraction failed")
                
                # ONLY use AI if traditional extraction failed
                if (not research_interests or len(research_interests) < 30) and self.ai_extractor:
                    ...
                    # Prepare page content for AI
                    for script in soup(["script", "style", "noscript"]):
                        script.decompose()
                    page_text = soup.get_text(" ")
                    page_text = re.sub(r'\s+', ' ', page_text).strip()
                    
                    ai_result = self.ai_extractor.extract_research_from_content(page_text, name)
                ...
                if (not research_interests or len(research_interests) < 30) and self.groq_extractor:
                    logger.info(f"Using Groq as final fallback for: {name}")
                    page_text = soup.get_text(" ")
                    groq_result = self.groq_extractor.extract_research_from_content(page_text, name)
                    if groq_result and groq_result != "Not found":
                        research_interests = groq_result
                        logger.info(f"Groq extraction successful: {len(research_interests)} chars")
...
def main():
    ...
    # AI arguments
    parser.add_argument('--use-ai', action='store_true', help='Use AI to extract/infer research (only as fallback)')
