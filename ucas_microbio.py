...
class ChineseUniversityFacultyScraper:
        ...
        for attempt in range(self.args.retries + 1):
            try:
                ...
                if template == 'TEMPLATE_1':
                    ...
                    # Try all methods
                    name = (self.extract_name_template1(soup) or 
                           self.extract_name_template2(soup) or 
                           self.extract_name_fallback(soup) or 
                           "Unknown")
                    email = (self.extract_email_template1(soup) or 
                            self.extract_email_template2(soup) or 
                            "Not found")
                    research = (self.extract_research_template1(soup) or 
    ...
    def run(self):
        ...
        profiles_list = []
        template_stats = {'TEMPLATE_1': 0, 'TEMPLATE_2': 0, 'UNKNOWN': 0, 'ERROR': 0}
