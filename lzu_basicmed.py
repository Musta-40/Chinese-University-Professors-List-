...
Updated to support Groq Cloud API.
"""
...
class AIExtractor:
    ...
    def __init__(self, provider='openai', api_key=None, model=None):
        ...
        if provider == 'openai':
            ...
            self.model_name = model or 'claude-3-haiku-20240307'
            ...
            self.model_name = model or 'llama-3.1-8b-instant'  # Fast and efficient default
        else:
        ...
        if provider == 'openai':
            ...
            if genai is None:
                raise RuntimeError("google-generativeai is not installed. pip install google-generativeai or choose another provider.")
            ...
            if Groq is None:
                raise RuntimeError("groq is not installed. Install it with: pip install groq")
    ...
    def extract_research_interests(self, page_content: str, faculty_name: str = "") -> str:
        ...
        try:
            if self.provider == 'openai':
                ...
                # Groq API is similar to OpenAI's new format
...
class SmartFacultyProfileScraper:
    ...
    def extract_email(self, soup) -> Optional[str]:
        ...
        pref = [e for e in emails if e.lower().endswith(('.edu.cn', '.edu')) or 'lzu.edu.cn' in e.lower()]
        if pref:
            return pref[0]
...
def main():
    parser = argparse.ArgumentParser(
        description='Smart Faculty Profile Scraper with AI Support (including Groq Cloud)'
    )
    ...
    parser.add_argument('--ai-provider', choices=['openai', 'anthropic', 'gemini', 'groq'], default='groq', 
    ...
    parser.add_argument('--ai-model', help='Model name (e.g., llama-3.1-70b-versatile, llama-3.1-8b-instant, mixtral-8x7b-32768 for Groq)')
