...
Updated for Northwestern University (NWU), Southwest University (SWU), and Jiangsu University (UJS)
"""
...
提取策略：
1. 首先查找这些关键词：Research Interest, Research Interests, Research focus, Research Works, 研究方向, 主要研究方向, 研究兴趣, 科研方向, 主要从事, 研究领域, Area of Research
2. 如果找到上述关键词，提取其后的研究描述内容（到下一个章节标题为止，如Education, Work Experience, Publications, Awards等）
3. 如果没有找到明确的研究方向部分，查找Publications, Research Projects, Papers等部分
...
Extraction Strategy:
1. First look for these keywords: Research Interest, Research Interests, Research focus, Research Works, Area of Research, 研究方向, 主要研究方向
2. If found, extract the research description that follows (until the next section like Education, Work Experience, Publications, Awards)
...
class SmartFacultyProfileScraper:
    ...
    # Research identifiers for Chinese sites (updated for UJS)
    ...
    # Research identifiers for English sites (updated for UJS)
    ...
    STOP_HEADERS_UJS = [
        ...
        # Chinese headers
        '工作经历', '公开发表文章', '获奖', '教学', '课程', '个人简介', '联系方式',
        '电话', '版权', '专业经历', '研究项目', '基金', '资助', '专利', '项目',
        '职业经历', '简历', '教育背景'
    ]
    ...
    def __init__(self, args):
        ...
        self.university_type = None  # 'NWU', 'SWU', 'UJS', or None
    ...
    def extract_publications_or_projects(self, soup) -> str:
        ...
        # Updated keywords for UJS format
            ...
            if heading:
                ...
                for elem in heading.find_next_siblings():
                    if elem.name in ['h1', 'h2', 'h3', 'strong']:
                        ...
                        if any(stop.lower() in elem_text.lower() for stop in self.STOP_HEADERS_UJS):
                            break
    ...
    def scrape_profile(self, url: str) -> Optional[Dict[str, str]]:
        ...
        for attempt in range(self.args.retries + 1):
            try:
                ...
                # For UJS, check if it's a faculty listing page (Source 1)
...
def main():
    parser = argparse.ArgumentParser(
        description='Faculty Profile Scraper for Chinese Universities (NWU, SWU & UJS)'
    )
