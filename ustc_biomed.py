...
Updated: expanded START_HEADING_VARIANTS (21 samples), extended STOP_HEADING_VARIANTS,
improved heading matching (substring match), stop on year/DOI or messy headings.
"""
...
class DualTemplateFacultyProfileScraper:
    ...
    # ---------- Updated start-heading variants (21 samples) ----------
    START_HEADING_VARIANTS = [
        # --- Original 16 Variants ---
        ...
        # --- NEWLY ADDED VARIANTS FROM SOURCES 17-21 ---
        'Main Research Directions',             # Source 17: WU Qingqing
        'Research Fields of Interests',         # Source 18: QU Kun
        'Research Interest',                    # Source 19: HUANG Chengdong
        '　RESEARCH INTERESTS',                 # Source 20: leading full-width space
        'Research Field of Interest:',          # Source 21: Jianye Zang (臧建业)
    ]

    # ---------- Updated stop-heading variants ----------
    STOP_HEADING_VARIANTS = [
        ...
        # --- NEW STOP KEYWORDS ---
        'Lab Recruitment',       # Source 17
        'Honors & Awards',       # Source 19
        'Job Opportunities',     # Source 18
        'Study Experience',      # user requested addition
    ]

    # Misc misleading headings (treat as hard-stop if encountered after start)
    MISLEADING_HEADINGS = [
    ...
    def is_messy_heading(self, h: Tag) -> bool:
        """
        Heuristic to detect the "messy heading" pattern (like:
        </span></span></p><p  ><br /></p><h3  ><span ><span lang="en-US"><strong><strong ><span >
        Stop if heading contains no readable text or only punctuation/non-word chars, or only <br> or empty spans.
        """
        try:
            raw_text = self.normalize_heading_text(h.get_text(separator=' ', strip=True))
            if not raw_text:
                # If decode_contents contains only tags and no text, treat as messy
                contents = (h.decode_contents() or '').strip()
                # If contents are tags only (no textual letters/digits), messy
                # Consider <br> or empty spans as messy
                if re.search(r'<br\s*/?>', contents, flags=re.IGNORECASE):
                    return True
                # If contents contain only tags and whitespace:
                if re.sub(r'<[^>]+>', '', contents).strip() == '':
                    return True
                return True
            # If text only contains punctuation / non-word characters, treat as messy
        ...
        try:
            for p in soup.find_all('p'):
                ...
                if re.search(r'\b(e-?mail|email|E-?mail|Email|邮箱)\b', text, flags=re.IGNORECASE):
                    ...
                    if m:
                        return m.group(1)
            return self.extract_email_from_contact_section(soup) or self.extract_email_from_content_div(soup) or re.search(
                r'([A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,})', soup.get_text(separator=' ', strip=True)
            ).group(1)
        except Exception:
    ...
    # ---------- Research extraction (content div consolidated) ----------
    def extract_research_from_content_div(self, soup) -> str:
        """
        Implements the consolidated extraction guide for <div class="content"> with <h3> headings.
        Uses substring-based matching for start and stop variants.
        """
        try:
            content_div = soup.find('div', class_='content')
            if not content_div:
            ...
            for h3 in content_div.find_all('h3'):
                ...
                # flexible substring-based start matching
                is_start = any(sv.strip().lower() in heading_text for sv in start_variants)
                if not is_start:
                    continue

                # Collect siblings until stop conditions
                collected_parts: List[str] = []
                for sibling in h3.find_next_siblings():
                    # If next heading encountered, check stop conditions
                    if isinstance(sibling, Tag) and sibling.name and sibling.name.lower() == 'h3':
                        ...
                        # Otherwise, continue (tolerate non-stop headings)
                        continue

                    # If not a heading, extract text and check internal stops
                    text = sibling.get_text(separator=' ', strip=True)
                    if not text:
                        # if it's an empty tag, skip
                        continue

                    # If the sibling contains a year or DOI at the start, stop (publication lists)
                    ...
                    # If sibling text contains a stop variant word, treat as stop (defensive)
            ...
            # Fallback: search paragraphs in content div for research keywords and capture a small block
    ...
    def extract_research_template1(self, soup) -> str:
        try:
            ...
            # Try content div consolidated logic and meta fallback
    ...
    def extract_research_template2(self, soup) -> str:
        try:
            ...
            # content-div consolidated logic
            cd = self.extract_research_from_content_div(soup)
            if cd:
                return cd
        ...
        for attempt in range(self.args.retries + 1):
            try:
                ...
                if template == 'TEMPLATE_1':
                    ...
                    research = self.extract_research_template1(soup) or self.extract_research_template2(soup) or self.extract_research_from_content_div(soup) or self.extract_from_meta_description(soup)
...
def main():
    parser = argparse.ArgumentParser(description='Faculty Profile Scraper with Dual Template Support (no AI)')
