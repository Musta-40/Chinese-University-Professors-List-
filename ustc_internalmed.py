...
def extract_research_interests(profile_soup):
    ...
    Looking for various headings related to research
    """
    research_interests = []
    ...
    if content_div:
        ...
        h3_tags = content_div.find_all(['h3', 'h2', 'h4'])
    ...
    # If no structured research section found, try alternative approach
    if not research_interests and content_div:
        # Look for paragraphs that contain research keywords
        all_text_elements = content_div.find_all(['p', 'li'])
        for elem in all_text_elements:
            text = elem.get_text(strip=True)
            if any(keyword in text.lower() for keyword in ['research', 'study', 'investigate', 'clinical trial', 'project']):
    ...
    if email == 'N/A':
        ...
        for e in email_patterns:
            if '@' in e and '.' in e.split('@')[1]:
                ...
                if not any(skip in e.lower() for skip in ['example.', 'email.', 'test.', 'sample.', 'user@']):
...
def extract_professor_name(profile_soup):
    ...
    if title_tag:
        ...
        if title_text and len(title_text) < 50 and title_text not in ['Faculty', 'Professor', 'Staff']:
            return title_text
...
def extract_department(profile_soup):
    """
    Extract department/specialty information
    """
    # Look for department info in content
    content_div = profile_soup.find('div', class_='content')
    if content_div:
        ...
        for pattern in dept_patterns:
            dept_match = re.search(pattern, content_text, re.IGNORECASE)
            if dept_match:
                return dept_match.group(1).strip()
    
    return 'Internal Medicine'

def scrape_internal_medicine_faculty():
    """
    Main function to scrape Internal Medicine faculty list
    """
    base_url = "https://enbiomed.ustc.edu.cn"
    list_url = "https://enbiomed.ustc.edu.cn/InternalMedicine/list.htm"
    ...
    try:
        print("\nFetching Internal Medicine faculty list page...")
        ...
        print(f"Found {len(bio_cards)} faculty members in Internal Medicine department")
        print("=" * 70)
        ...
        for i, card in enumerate(bio_cards, 1):
            ...
            if profile_link_elem and profile_link_elem.get('href'):
                ...
                try:
                    ...
                    professor_info['department'] = extract_department(profile_soup)
                    ...
                    professor_info['department'] = 'Internal Medicine'
...
def save_to_csv(data, filename='internal_medicine_professors.csv'):
    """
    ...
    fieldnames = ['name', 'title', 'department', 'email', 'profile_link', 'research_interests']
...
def save_to_txt(data, filename='internal_medicine_professors.txt'):
    """
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        f.write("INTERNAL MEDICINE FACULTY - USTC FIRST AFFILIATED HOSPITAL\n")
        f.write("=" * 80 + "\n\n")
        
        for i, prof in enumerate(data, 1):
            f.write(f"{i}. {prof['name']}\n")
            f.write(f"   Title: {prof.get('title', 'N/A')}\n")
            f.write(f"   Department: {prof.get('department', 'Internal Medicine')}\n")
...
def save_to_json(data, filename='internal_medicine_professors.json'):
    """
...
def print_summary_stats(data):
    ...
    # Count by department/specialty
    depts = {}
    for prof in data:
        dept = prof.get('department', 'Internal Medicine')
        depts[dept] = depts.get(dept, 0) + 1
    
    if len(depts) > 1:
        print("\nFaculty by Specialty/Department:")
        for dept, count in sorted(depts.items(), key=lambda x: x[1], reverse=True):
            print(f"  • {dept}: {count}")

def main():
    print("\n" + "=" * 70)
    print(" " * 10 + "USTC INTERNAL MEDICINE FACULTY WEB SCRAPER")
    print("=" * 70 + "\n")
    ...
    # Scrape the data
    professors_data = scrape_internal_medicine_faculty()
    ...
    if professors_data:
        ...
        if successful:
            ...
            for prof in successful[:3]:
                ...
                print(f"  Department: {prof.get('department', 'Internal Medicine')}")
